...
const BoppTape = ({ brand, product, category, brandId, subCategoryId, q }) => {
  ...
  const [breadth, setBreadth] = useState([0, 100]);
  ...
  const handleApply = async () => {
    ...
    if (flags?.category) {
      ...
      const filteredProducts = dat?.data?.data.filter((product) => {
        return product.brand === "6557dbad301ec4f2f4266103";
      });
  ...
  useEffect(() => {
    ...
    if (product) {
      const boppTapeProducts = product.filter(
        (product) => product.brand?.name === "Amazon"
      );
    ...
    if (flags?.size) {
      ...
      if (index === -1) {
        ...
        const data = {
          ...
          category: temp,
          brand: "6557dbad301ec4f2f4266103", // Add the brand filter here
  ...
  return (
    <>
      <Head>
        <title>Buy Amazon Corrugated Boxes Online | store.prempackaging</title>
        <meta name="title" content="Buy Amazon Corrugated Boxes Online" />
        <meta
          name="description"
          ...
          >
            <div className="mt-2 d-flex flex-column">
              <p className={styles.catalogpath}>
                ...
                <span className={styles.spanee}>Amazon</span>
              </p>
              <div
