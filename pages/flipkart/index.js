  ...
  const handleApply = async () => {
    ...
    if (flags?.category) {
      ...
      const filteredProducts = dat?.data?.data.filter((product) => {
        return product.brand === "6557dbbc301ec4f2f4266107";
      });
  ...
  useEffect(() => {
    ...
    if (product) {
      const boppTapeProducts = product.filter(
        (product) => product.brand?.name === "Flipkart"
      );
    ...
    if (flags?.size) {
      ...
      if (index === -1) {
        ...
        const data = {
          ...
          category: temp,
          brand: "6557dbbc301ec4f2f4266107",
        ...
        const data = {
          ...
          category: temp,
          brand: "6557dbbc301ec4f2f4266107", // Add the brand filter here
          ...(subCategoryId !== null && { subcategory: subCategoryId }),
        };
        const dat = await postService("/product/filter", data);
        setProducts(dat?.data?.data);
          ...
          >
            <div className="mt-2 d-flex flex-column">
              <p className={styles.catalogpath}>
                ...
                <span className={styles.spanee}>Flipkart</span>
              </p>
              <div
            ...
            <div className="row mt-4" style={{ position: "relative" }}>
              ...
              <div
                className={"col-9 p-0 w-100 " + styles.productslistdivmobile}
              >
                {products && products.length > 0 ? (
                  products.slice(firstIndex, lastIndex).map((item, index) => (
                    <div
