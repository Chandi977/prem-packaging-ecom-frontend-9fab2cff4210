...
const BoppTape = ({ brand, product, category, brandId, subCategoryId, q }) => {
    ...
    try {
      ...
      if (flags?.category) {
        ...
        const filteredProducts = dat?.data?.data.filter((product) => {
          return product.category === "6557df64301ec4f2f4266141";
        });
  ...
  useEffect(() => {
    ...
    if (product) {
      const boppTapeProducts = product.filter(
        (product) => product.category?.name === "Carry Handle Tape"
      );
    ...
    if (flags?.size) {
      ...
      if (index === -1) {
        ...
        const filteredProducts = dat?.data?.data?.filter(
          (product) => product.category === "6557df64301ec4f2f4266141"
        );
        setProducts(filteredProducts);
      } else {
        temp.splice(index, 1);
  ...
  return (
    <>
      <Head>
        <title>Buy Carry Handle Tapes online | store.prempackaging </title>
        <meta name="title" content="Buy Carry Handle Tapes online" />
        <meta
          name="description"
          content="Durable PackPro Carry Handle Tape designed for convenience and strength. Easily lifts up to 15kg, making packaging and transportation effortless. Order now!"
        />
      </Head>
      <div style={{ paddingTop: `${widt > breakpoint ? "120px" : "150px"}` }}>
        {isLoading && <Loader />}
        <div className="row p-0 m-0">
          <CarryHandleTapeBanner />
          <div
          ...
          >
            <div className="mt-2 d-flex flex-column">
              <p className={styles.catalogpath}>
                ...
                <span className={styles.spanee}>
                  PackPro Carry Handle Tapes
                </span>
              </p>
              <div
