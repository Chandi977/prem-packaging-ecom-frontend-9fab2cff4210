    ...
    try {
      ...
      if (flags?.category) {
        ...
        const filteredProducts = dat?.data?.data.filter((product) => {
          return product.category === "689d73214687bb4e437542e0";
        });
  ...
  useEffect(() => {
    ...
    if (product) {
      const boppTapeProducts = product.filter(
        (product) => product.category?.name === "Carry Bag"
      );
    ...
    if (flags?.size) {
      ...
      if (index === -1) {
        ...
        const filteredProducts = dat?.data?.data?.filter(
          (product) => product.category === "689d73214687bb4e437542e0"
        );
        setProducts(filteredProducts);
      } else {
        temp.splice(index, 1);
          ...
          >
            <div className="mt-2 d-flex flex-column">
              <p className={styles.catalogpath}>
                ...
                <span className={styles.spanee}>Carry Bags</span>
              </p>
              <div
