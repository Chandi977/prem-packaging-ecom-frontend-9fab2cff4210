...
const BoppTape = ({ brand, product, category, brandId, subCategoryId, q }) => {
    ...
    try {
      ...
      if (flags?.category) {
        ...
        const filteredProducts = dat?.data?.data.filter((product) => {
          return product.category === "6642e8f665f20fe41ab417bc";
        });
  ...
  useEffect(() => {
    ...
    if (product) {
      const paperTapeProducts = product.filter(
        (product) => product.category?.name === "Speciality Tape"
      );
      // console.log(boppTapeProducts);
      //console.log(product);
      setProducts(paperTapeProducts);
    }
  }, [product]);
    ...
    if (flags?.size) {
      ...
      if (index === -1) {
        ...
        const filteredProducts = dat?.data?.data?.filter(
          (product) => product.category === "6642e8f665f20fe41ab417bc"
        );
        setProducts(filteredProducts);
      } else {
        temp.splice(index, 1);
  ...
  return (
    <>
      <Head>
        <title>Secure Tamper Evident Void Tape | store.prempackaging </title>
        <meta name="title" content="Secure Tamper Evident Void Tape" />
        <meta
          name="description"
          content="Shop high-quality void tape at store.prempackaging.com. Ensure tamper-evident security for your packages with durable, reliable, and cost-effective packaging solutions."
        />
      </Head>
      <div style={{ paddingTop: `${widt > breakpoint ? "120px" : "150px"}` }}>
        {isLoading && <Loader />}
        <div className="row p-0 m-0">
          <SpecialityTapeBanner />
          <div
          ...
          >
            <div className="mt-2 d-flex flex-column">
              <p className={styles.catalogpath}>
                ...
                <span className={styles.spanee}>Void Tapes </span>
              </p>
              <div
            ...
            <div className="row mt-4 d-flex" style={{ position: "relative" }}>
              {/* Desktop Filter */}
              <div
                className={"d-flex flex-column col-3 " + styles.mobileFilter}
              >
              ...
              {/*Mobile Filter  */}
              <div
                className={"d-flex flex-column col-12 " + styles.desktopFilter}
              >
                <div className={"col-12 p-0 m-0" + styles.filterslayout}>
                  <div
                    className="w-100"
